#!/usr/bin/env python3
"""Build and verify the OASI v0.4.1 terminology-corrected article reproducibly.
...
FIXED_BUILD_TIME_UTC = "2026-09-03T00:00:00Z"
ARTICLE_TITLE = (
...
ARTICLE_REL = "paper/v0.4.1/OASI_SCIENTIFIC_ARTICLE_PREPRINT_V0_4_1.pdf"
SOURCE_MANIFEST_REL = "paper/v0.4.1/ARTICLE_SOURCE_MANIFEST.sha256"
RECEIPT_REL = "paper/v0.4.1/BUILD_RECEIPT.json"
SOURCE_PREFIX = PurePosixPath("paper/v0.4.1/source")
FIGURE_NAMES = (
...
NORMATIVE_SOURCE_RELS = (
    ...
    *(f"paper/v0.4.1/source/figures/{name}.tex" for name in FIGURE_NAMES),
)
...
def source_snapshot(root: Path, phase: str, expected_article_doi: str | None) -> dict[str, bytes]:
    ...
    main_text = snapshot["paper/v0.4.1/source/main.tex"].decode("utf-8")
    ...
    if phase == "pre-doi":
        ...
        if expected_article_doi in HISTORICAL_DOIS:
            raise BuildFailure("final v0.4.1 DOI cannot reuse any prior OASI DOI")
...
def build_once(
    ...
    for name in FIGURE_NAMES:
        ...
        pdf_rel = f"paper/v0.4.1/source/figures/{name}.pdf"
        svg_rel = f"paper/v0.4.1/source/figures/{name}.svg"
